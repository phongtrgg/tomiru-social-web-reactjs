...
import { getTweetById, selectLoadingState } from "../../store/ducks/tweets/selectors";
...
const AddTweetForm: FC<AddTweetFormProps> = ({
    ...
    const { text, setText, handleChangeText, textConverter } = useInputText();
    ...
    const handleClickAddTweet = async (): Promise<void> => {
        ...
        // tweetPostProcessing(
        //     scheduledDate ? `Your Tweet will be sent on ${formatScheduleDate(scheduledDate)}` : t("your_tweet_was_sent")
        // );
    };
    ...
    const tweetPreProcessing = async () => {
        ...
        for (const image of images) {
            ...
            const { data } = await TweetApi.uploadMultiTweetImage(formData);
            result = data;
        }
    ...
    const tweetPostProcessing = (snackBarText?: string): void => {
        // dispatch(setOpenSnackBar(snackBarText ?? t("your_tweet_was_sent")));
