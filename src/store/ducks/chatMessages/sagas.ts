...
//add chatGroup-message
export function* addChatMessageGroupRequest({ payload }: AddChatMessageActionInterface) {
    try {
        yield call(ChatMessageApi.addMessageGroup, payload);
    } catch (error) {
        yield put(setChatMessagesLoadingState(LoadingStatus.ERROR));
    }
}
...
export function* chatMessagesSaga() {
    ...
    yield takeLatest(ChatMessagesActionsType.ADD_CHAT_GROUP_MESSAGE, addChatMessageGroupRequest);
}
