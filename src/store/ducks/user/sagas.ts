...
import {
    ...
    UpdateUserAboutActionInterface,
...
import { ChangePhoneResponse, EditAboutRequest } from "./contracts/state";
import {
...
import { unfollowFriendResponse } from "../friendUser/actionCreators";
...
export function* updateUserDataRequest({ payload }: UpdateUserDataActionInterface) {
    try {
        ...
        yield put(setHaveSentEditRequest(true));
    } catch (error) {
        yield put(setUserLoadingStatus(LoadingStatus.ERROR));
    }
}
export function* updateUserAboutRequest({ payload }: UpdateUserAboutActionInterface) {
    try {
        yield put(setUserLoadingStatus(LoadingStatus.LOADING));
        const response: AxiosResponse<AuthUserResponse> = yield call(UserApi.updateUserAbout, payload);
        yield put(setUserData(response.data));
    } catch (error) {
        yield put(setUserLoadingStatus(LoadingStatus.ERROR));
    }
...
export function* processFollowUserRequest({ payload }: FollowUserActionInterface) {
    try {
        ...
        yield put(unfollowFriendResponse(payload.userId, data));
    } catch (error) {
        yield put(setUserLoadingStatus(LoadingStatus.ERROR));
    }
}
...
export function* processUserToBlocklistRequest({ payload }: ProcessUserToBlocklistActionInterface) {
    try {
        ...
        yield put(
            setBlockedUsersTweetState({ userId: payload.userId, tweetId: payload.tweetId!, isUserBlocked: data })
        );
        yield put(
            setBlockedToListTweetsState({ userId: payload.userId, tweetId: payload.tweetId!, isUserBlocked: data })
        );
...
export function* processFollowRequests({ payload }: ProcessFollowRequestActionInterface) {
    try {
        const { data }: AxiosResponse<UserProfileResponse> = yield call(
...
export function* userSaga() {
    ...
    yield takeLatest(UserActionsType.UPDATE_ABOUT, updateUserAboutRequest);
}
