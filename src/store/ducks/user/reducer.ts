...
export const userReducer = produce((draft: Draft<UserState>, action: UserActions) => {
    switch (action.type) {
        ...
        case UserActionsType.SET_USER_FOLLOWING:
            if (draft.data) {
                draft.data.followersCount = action.payload
                    ? draft.data.followersCount + 1
                    : draft.data.followersCount - 1;
                draft.status = LoadingStatus.LOADED;
            }
            break;
        ...
        case UserActionsType.SET_USER_LOADING_STATE:
            ...
            break;
        case UserActionsType.SET_HAVE_SENT_EDIT_REQUEST:
            draft.haveSentEditRequest = action.payload;
            break;
        default:
            break;
    }
