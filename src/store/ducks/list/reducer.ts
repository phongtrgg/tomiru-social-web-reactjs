    ...
    switch (action.type) {
        ...
        case ListActionType.SET_UPDATED_LIST_TWEET:
            if (action.payload.notificationType === NotificationType.LIKE) {
                ...
                if (repliedTweetIndex !== -1)
                    draft.tweets[repliedTweetIndex].repliesCount = draft.tweets[repliedTweetIndex].repliesCount + 1;
            }
            break;
        ...
        case ListActionType.SET_UPDATED_BOOKMARKED_LIST_TWEET:
            ...
            if (bookmarkedTweetIndex !== -1)
                draft.tweets[bookmarkedTweetIndex].isTweetBookmarked = action.payload.isTweetBookmarked;
            break;
        ...
        case ListActionType.SET_FOLLOW_TO_LIST_TWEETS_STATE:
            if (action.payload.tweetId) {
                ...
                if (followUserTweetIndex !== -1)
                    draft.tweets[followUserTweetIndex].author.isFollower = action.payload.isFollower;
            } else {
        ...
        case ListActionType.SET_BLOCKED_TO_LIST_TWEETS_STATE:
            if (action.payload.tweetId) {
                ...
                if (blockedUserTweetIndex !== -1)
                    draft.tweets[blockedUserTweetIndex].author.isUserBlocked = action.payload.isUserBlocked;
            } else {
                draft.tweets = draft.tweets.map((tweet) => {
                    if (tweet.author.id === action.payload.userId) {
        ...
        case ListActionType.SET_MUTED_TO_LIST_TWEETS_STATE:
            if (action.payload.tweetId) {
                ...
                if (mutedUserTweetIndex !== -1)
                    draft.tweets[mutedUserTweetIndex].author.isUserMuted = action.payload.isUserMuted;
            } else {
                draft.tweets = draft.tweets.map((tweet) => {
                    if (tweet.author.id === action.payload.userId) {
        ...
        case ListActionType.SET_TWEETS_LOADING_STATE:
            ...
            break;
        case ListActionType.RESET_STATUS:
            draft.loadingState = LoadingStatus.LOADED;
            break;
        case ListActionType.OUT_GROUP_SUCCESS:
