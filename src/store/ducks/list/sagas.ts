...
import { outListSuccess, setList, setListLoadingState, setListTweets, setTweetsLoadingState } from "./actionCreators";
...
export function* outGroupRequest({ payload }: OutListActionInterface) {
    try {
        yield put(setListLoadingState(LoadingStatus.LOADING));
        const response: AxiosResponse<BaseListResponse> = yield call(ListsApi.outGroup, payload);
        yield put(outListSuccess(response.data));
    } catch (error) {
...
export function* listSaga() {
    ...
    yield takeEvery(ListActionType.OUT_GROUP, outGroupRequest);
}
