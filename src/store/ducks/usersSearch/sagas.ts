...
//update
export function* fetchUserByUserNameRequest({ payload }: FetchUsersSearchByNameActionInterface) {
    try {
        yield put(setUsersSearchLoadingState(LoadingStatus.LOADING));
        const response: AxiosResponse<UserResponse[]> = yield call(esearchApi.getUsersByUserName, payload);
        yield put(
...
export function* usersSearchSaga() {
    ...
    //
    yield takeLatest(UsersSearchActionsType.FETCH_USERS_BY_NAME, fetchUserByUserNameRequest);
    //
