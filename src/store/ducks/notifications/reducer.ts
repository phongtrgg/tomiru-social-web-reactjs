...
export const notificationsReducer = produce((draft: Draft<NotificationsState>, action: NotificationsActions) => {
    switch (action.type) {
        ...
        case NotificationsActionsType.SET_TWEET_AUTHORS_LOADING_STATE:
            ...
            break;
        case NotificationsActionsType.SET_NOTIFICATION_AS_READ:
            const newList = [...draft.notificationsList].map((n) =>
                n.id === action.payload ? { ...n, read: true } : n
            );
            draft.notificationsList = newList;
            draft.notificationInfoLoadingState = LoadingStatus.LOADED;
            break;
        case NotificationsActionsType.SET_ALL_NOTIFICATIONS_AS_READ:
            draft.notificationsList = [...draft.notificationsList].map((n) => ({ ...n, read: true }));
            draft.notificationInfoLoadingState = LoadingStatus.LOADED;
            break;
        case NotificationsActionsType.DELETE_NOTIFICATION:
            draft.notificationsList = [...draft.notificationsList].filter((n) => n.id !== action.payload);
            draft.notificationInfoLoadingState = LoadingStatus.LOADED;
            break;
        default:
            break;
