...
import {
    ...
    resetTweets,
    fetchTweetByTagNameRequest,
    fetchTweetByTextRequest
...
const Explore: FC = (): ReactElement => {
    ...
    useEffect(() => {
        ...
        if (location.state?.tag) {
            ...
            fetchTweetsByStateText(fetchTweetByTagNameRequest({ tag: location.state.tag, pageNumber: 0 }), location.state.tag);
        }
        if (location.state?.text) {
            // fetchTweetsByStateText(fetchTweetsByText({ text: location.state.text, pageNumber: 0 }), location.state.text);
            fetchTweetsByStateText(fetchTweetByTextRequest({ text: location.state.text, pageNumber: 0 }), location.state.text);
        }
    ...
    const loadTweets = (): void => {
        if (searchText) {
            ...
            if (activeTab !== 2) {
                ...
                dispatch(fetchTweetByTextRequest({ text: encodedText, pageNumber }));
            } else {
                // dispatch(fetchUsersSearchByUsername({ username: encodedText, pageNumber }));
                dispatch(fetchUserByUserNameRequest({ username: encodedText, pageNumber }));
            }
        } else {
            if (activeTab === 2) {
                ...
                <div>
                    ...
                    <div className={classes.tabs}>
                        <Tabs value={activeTab} onChange={handleChangeTab} indicatorColor="primary" textColor="primary">
                            <Tab onClick={() => handleShowItems(showTopTweets)} label={t("top")} />
                            <Tab onClick={() => handleShowItems(showTopTweets)} label={t("latest")} />
                            <Tab onClick={() => handleShowItems(showUsers)} label={t("people")} />
                            <Tab onClick={() => handleShowItems(showMediaTweets)} label={t("photos")} />
                            <Tab onClick={() => handleShowItems(showTweetsWithVideos)} label={t("videos")} />
                        </Tabs>
                    </div>
                </div>
            </PageHeaderWrapper>
